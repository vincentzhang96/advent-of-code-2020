...
fn check_password(s: &str) -> bool {
    ...
    let mut count = 0;
    for pwletter in pw.chars() {
        if pwletter == letter {
            count += 1;
            
            // Bail early if we're over
            if count > max {
                return false;
            }
        }
...
fn get_rule(s: &str) -> (i32, i32, char) {
    ...
    let lens = split[0];
    let letter = split[1];
    let lensplit: Vec<&str> = lens.split("-").collect();
    let min: i32 = lensplit[0].parse().unwrap();
    let max: i32 = lensplit[1].parse().unwrap();
    return (min, max, letter.chars().next().unwrap());
}


static INPUT: &str = "1-14 b: bbbbbbbbbbbbbbbbbbb
